...
function loadCiliaHubData() {
  ...
  searchBtn.addEventListener('click', () => {
    ...
    let filteredData = data.filter(item => {
      const gene = (item.gene || '').toLowerCase();
      const ensembl = (item.ensembl_id || '').toLowerCase();
      const omim = (item.omim_id || '').toLowerCase();
      const synonyms = (item.synonym || '').toLowerCase().split(/[,; ]+/).map(s => s.trim());

      return (
        gene.includes(query) ||
        ensembl.includes(query) ||
        omim.includes(query) ||
